...
function renderTrainings(){
  ensurePhase2Data();
  const cards=db.myWorkouts.filter(w=>w.active!==false).map(w=>`<article class="training-card f2-card ${f2LevelClass(w.level)}" onclick="startWorkoutById('${w.id}')" role="button" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();startWorkoutById('${w.id}')}">
    <div class="card-top"><span class="badge">${esc(f2DisplayName(w.level||"Personalizado"))}</span><span class="exercise-count">${workoutExerciseCount(w)} exercícios</span></div>
    <button class="training-delete" onclick="event.stopPropagation();deleteMyWorkout('${w.id}')" aria-label="Excluir treino" title="Excluir treino">🗑</button>
    <h3>${esc(f2DisplayName(w.name))}</h3><p>${esc(w.description||w.groups.map(g=>f2DisplayName(g.name)).join(" • ")||"Sem grupos definidos")}</p>
    <div class="card-actions"><button class="primary" onclick="event.stopPropagation();startWorkoutById('${w.id}')">▶ Iniciar</button><button class="secondary" onclick="event.stopPropagation();editMyWorkout('${w.id}')">✎ Editar</button></div>
  </article>`).join("");
  layout(`<div class="page-title-row"><div><p class="muted">Organize seus treinos por grupos musculares, exercícios e séries.</p></div><button class="primary small" onclick="createMyWorkout()">＋ Treino</button></div>
    <div class="training-grid">${cards||'<div class="empty big">Nenhum treino ativo.</div>'}</div>
    <section class="settings-card"><h3>Gerenciar treinos</h3><div class="list">${db.myWorkouts.map(w=>`<div class="list-card"><span class="badge">${w.active===false?'OFF':esc(f2DisplayName(w.level||''))}</span><div><b>${esc(f2DisplayName(w.name))}</b><small>${workoutExerciseCount(w)} exercícios • ${w.groups.length} grupos</small></div><button class="iconbtn" onclick="editMyWorkout('${w.id}')" aria-label="Editar treino" title="Editar treino">✎</button><button class="iconbtn" onclick="duplicateMyWorkout('${w.id}')" aria-label="Duplicar treino" title="Duplicar treino">⧉</button><button class="iconbtn" onclick="toggleMyWorkout('${w.id}')" aria-label="${w.active===false?'Ativar':'Desativar'} treino" title="${w.active===false?'Ativar':'Desativar'} treino">${w.active===false?'▶':'⏸'}</button></div>`).join("")}</div></section>` ,"trainings");
}
